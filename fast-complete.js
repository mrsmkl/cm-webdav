...
exports.augmentedLetterComplete = function (o_str, pat) {
    var vals = new MultiSet();
	var keys = new MultiSet();
	for (var i = 0; i < letters.length; i++) {
		var str = o_str + letters[i];
		var cell = augmentedCell(str, pat);
		if (!cell || !cell.str || cell.val <= 0) continue;
		keys.set(cell.str, letters[i]);
		vals.add(cell.str, cell.val);
	}
    // Sort...
	...
	return {list:res, current:augmentedCell(o_str)};
}
...
function augmentedCell(str, pat) {
    var cell = current.getValue(str);
    if (!cell) {
        var ind = str.split(pat)[0].length;
        if (ind == str.length) return str;
        var pat_length = str.substr(ind).match(pat)[0].length;
        return str.substr(0,ind+pat_length) + augmentedCell(str.substr(ind+pat_length), pat);
    }
    return cell;
};
